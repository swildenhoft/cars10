...
#returns the SteamID from steamid.json file
...
#bot command !stats, shows all time stats pulled from steams API.
@client.command()
async def stats(ctx):
...
#bot command !stats, shows last match's stats pulled from steams API.    
@client.command()
